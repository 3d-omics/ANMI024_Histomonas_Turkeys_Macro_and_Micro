...
```{r plot_beta_macro_order_composition, warning=FALSE, comments="", message=FALSE, fig.height=10, fig.width=10}
barplot_absolute <- tidy_plot_genome_counts_filt_30_macro %>%
  ...
  labs(x = NULL, y = "Order composition (Genome counts)",fill = "Read type") +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 10),
        ...
        plot.margin = margin(0.2, 0.2, 5, 0.2)) #+
  #ggtitle("Composition of bacterial orders at 30% coverage filtered data")

barplot_relative <- tidy_plot_genome_counts_filt_30_macro_closed %>%
  filter(treatment != "TM0") %>%
  ggplot(aes(x = animal , y = count, fill = order)) +
  geom_bar(stat="identity", colour="white", linewidth=0.1) + #plot stacked bars with white borders
  scale_fill_manual(values = order_colors[-4], drop = FALSE) +
  labs(x = "Metagenomic Samples (Animal ID)", y = "Order composition (Relative abundance)",fill = "Read type") +
  facet_nested(. ~ treatment_expl + age_category, scales = "free", space = "free") +
  theme_minimal() + 
  custom_ggplot_theme +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 10),
        axis.text.y = element_text(),
        legend.position = "none",
        plot.margin = margin(5, 0.2, 0.2, 0.2)) #+
  #ggtitle("Composition of bacterial orders at 30% coverage filtered data")

barplot_absolute / barplot_relative
```
